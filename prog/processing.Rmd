...
labels_table <- melt(setDT(labels), measure = patterns(c("^dates_out","^sites","^site_settings")),
     value.name = c("Timestamp", "Site", "LTreatment"))
labels_table <- labels_table[,.SD,
             .SDcols = c("Timestamp", "Site", "LTreatment")]
...
labels_table[status_table, on = c("Timestamp","Site"), Status := i.Status]
```
...
### Remove bad/corrupted files where Site is 0; sort by time
```{r}
...
Anything that is NA should be removed (checked).
```{r}
...
# changepoints <- big_table[rowid(rleid(Treatment)) == 1]
# changepoints$Timestamp <- as.Date(changepoints$Timestamp)
```
...
dcast(big_table, Site + Folder ~ Timestamp, fun=get_min_diff,
      value.var="Timestamp")
...
diag_table <- data.table(
  site = big_table[, by=Site, is.unsorted(Timestamp)]$Site,
