...
big_table[LabelledSite==1]

#big_table[, rowid(rleid(Treatment)) == 1, by=LabelledSite]

# here we get the row indices of every change point by site
# have to manually handle first item

big_table[ , Timestamp[Treatment != shift(Treatment, 1L, type="lag")
            | Timestamp %in% big_table[!duplicated(big_table$LabelledSite)]$Timestamp
            ], by=LabelledSite]
