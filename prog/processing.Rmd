...
labels_table$Timestamp <- as.POSIXct(labels_table$Timestamp)
...
#dim(big_table[, !duplicated(LabelledSite), by=.(LabelledSite)])
#DT[, NEWCLS := as.integer(!duplicated(CLASS)), by = ID]

#str(big_table[, !duplicated(LabelledSite), by=.(LabelledSite,Folder)])
str(big_table[!duplicated(big_table[,c("LabelledSite","Folder"),with=F])])
 #dim(,big_table[!duplicated(.SD, by=c("LabelledSite", "Folder")),
...
big_table[Treatment != shift(Treatment, 1L, type="lag") | Timestamp %in% big_table[!duplicated(big_table[,c("LabelledSite","Folder"),with=F])]$Timestamp, 
    StartDate := as.Date(Timestamp), by=.(LabelledSite,Folder)]
...
changepoints <- big_table[ , Timestamp[Treatment != shift(Treatment, 1L, type="lag") 
                                      | Timestamp %in% big_table[!duplicated(big_table[,c("LabelledSite","Folder"),with=F])]$Timestamp], 
...
ggplot() +
  geom_point(data=big_table[Timestamp < as.Date("2023-07-01")], aes(x=Timestamp,y=Treatment,colour=as.factor(Folder))) +
  geom_point(data=labels_table[Timestamp < as.Date("2023-07-01")], aes(x=Timestamp,y=LTreatment,alpha=0.5)) +
  geom_point(data=changepoints[Timestamp < as.Date("2023-07-01")], aes(x=Timestamp,y=Treatment,colour="red",alpha=0.5)) +
  scale_x_datetime(
